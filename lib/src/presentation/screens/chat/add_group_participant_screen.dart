...
class AddGroupParticipantScreen extends StatefulWidget {
  ...
  const AddGroupParticipantScreen({super.key, required this.session});

  @override
  State<AddGroupParticipantScreen> createState() =>
      _AddGroupParticipantScreenState();
}

class _AddGroupParticipantScreenState extends State<AddGroupParticipantScreen> {
  List<User> users = [];
      ...
      result.fold(
        ...
        (data) {
          ...
          final existingUserIds =
              widget.session.participants.map((p) => p.user.id).toSet();
          final availableUsers =
              newUsers.where((u) => !existingUserIds.contains(u.id)).toList();
      ...
      result.fold(
        ...
        (data) {
          ...
          setState(() {
            ...
            filteredUsers.addAll(availableUsers);
            hasMore = data['hasMore'] as bool;
            isLoadingMore = false;
          });
        },
        ...
        (_) {
          if (mounted) {
            ScaffoldMessenger.of(context).showSnackBar(
              SnackBar(
                content: Text(
                  '${user.fullName ?? user.username} added to group',
                ),
